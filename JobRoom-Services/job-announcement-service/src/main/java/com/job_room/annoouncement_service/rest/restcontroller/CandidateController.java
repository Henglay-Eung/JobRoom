...
public class CandidateController {
    ...
    public ResponseEntity<BaseApiResponse<CandidateResponse>> insertCandidate(
        ...
        Company company=announcementService.companyById(candidateRequest.getCompanyId());

        if (result == null) {
            response.setMessage(messageProperties.insertError("candidate (announcement or employee or company not found)"));
    ...
    public ResponseEntity<Object> filterByAnnouncementId(
            ...
            @RequestParam(defaultValue = "10") int pageSize) {
        Pageable pageable = PageRequest.of(page, pageSize);
//        BaseApiResponseWithPagination<AnnouncementCandidates> baseApiResponseWithPagination=new BaseApiResponseWithPagination<>();
//        AnnouncementCandidates announcementCandidates=new AnnouncementCandidates();
//        List<CandidateResponseList> candidateResponseLists=new ArrayList<>();
//        Pagination pagination=new Pagination();
//        AnnouncementDto announcementDto = announcementService.selectById(announcementId);
//        if (announcementDto != null) {
//            announcementCandidates = modelMapper.map(announcementDto, AnnouncementCandidates.class);
//            Page<CandidateDto> candidateDtos = candidateService.filterCandidateByAnnouncementId(announcementId, pageable);
//
//            for (int i = 0; i <candidateDtos.getContent().size() ; i++) {
//                candidateResponseLists.add(modelMapper.map(candidateDtos.getContent().get(i),CandidateResponseList.class));
//                CandidateEmployee candidateEmployee = candidateService.selectEmployeeById(candidateDtos.getContent().get(i).getEmployeeId());
//                candidateResponseLists.get(i).setEmployee(candidateEmployee);
//                announcementCandidates.setCandidate(candidateResponseLists);
//            }
//
//            pagination.setCurrentPage(candidateDtos.getNumber());
//            pagination.setPageSize(candidateDtos.getSize());
//            pagination.setTotalItems(candidateDtos.getTotalElements());
//            pagination.setTotalPages(candidateDtos.getTotalPages());
//
//
//            baseApiResponseWithPagination.setTime(new Timestamp(System.currentTimeMillis()));
//            baseApiResponseWithPagination.setMessage(messageProperties.selected("candidate"));
//            baseApiResponseWithPagination.setData(announcementCandidates);
...
//        }else{
//            baseApiResponseWithPagination.setTime(new Timestamp(System.currentTimeMillis()));
//            baseApiResponseWithPagination.setMessage(messageProperties.hasNoRecords("candidate"));
//            baseApiResponseWithPagination.setData(null);
//            baseApiResponseWithPagination.setPagination(pagination);
//            baseApiResponseWithPagination.setStatus(HttpStatus.NOT_FOUND);
//            return ResponseEntity.ok(baseApiResponseWithPagination);
//        }
    ...
    @GetMapping("/candidates/company/{id}/filterDate")
    public ResponseEntity<Object> filterByDateAndCompanyId(
            @PathVariable int id,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int pageSize,
        ...
        try {
            ...
            if (dateValidator.isValid(startDate) && dateValidator.isValid(endDate)) {
                ...
                Page<CandidateDto> candidatePage = candidateService.findAllByCompanyIdAndAppliesDateBetween(id,date1, date2, pageable);
                return filter(candidatePage);
            } else {
    ...
    @GetMapping("/candidates/count/{companyId}")
    public ResponseEntity<BaseApiResponse<Map<String, Integer>>> countCandidateWithActiveAnnouncement(
            @PathVariable int companyId
    ) {
        Map<String, Integer> map = new HashMap<>();
        BaseApiResponse<Map<String, Integer>> baseApiResponse = new BaseApiResponse();
        int count = candidateService.countAllByCompanyId(companyId);

        map.put("count", count);
        baseApiResponse.setData(map);
        baseApiResponse.setMessage("count candidate successfully!");
    ...
    @GetMapping("/candidates/company/{id}")
    public ResponseEntity<Object> findAllByCompanyId(
            @PathVariable int id,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int pageSize) {

                Pageable pageable = PageRequest.of(page, pageSize, Sort.by("id").descending());
                Page<CandidateDto> candidateDtos =candidateService.findAllByCompanyId(id,pageable);
                return filter(candidateDtos);
            }
    @GetMapping("/candidates/company/{id}/search")
    public ResponseEntity<Object> searchCandidateByName(
            @PathVariable int id,
        ...
        Pageable pageable = PageRequest.of(page, pageSize, Sort.by("id").descending());
        Page<CandidateDto> candidateDtos =candidateService.SearchCandidateByName(id,name,pageable);
        return filter(candidateDtos);
    }
            ...
            if (candidateDtoList.size() <= 0) {
                ...
                for (int i = 0; i < candidateDtoList.size(); i++) {
                    System.out.println(candidateDtoList.get(i).getName());
