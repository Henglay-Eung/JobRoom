...
public class AnnouncementController {
    ...
    // user can see announcement that is not banned
    ...
    //TODO: Select announcement by company id for Admin  =========================================================
    //Note admin can see all announcement
    @GetMapping("/announcements/company/admin/{companyId}")
    public ResponseEntity<Object> selectAnnouncementByCompanyIdForAdmin(
            @PathVariable int companyId,
            @RequestParam(defaultValue = "0", required = false) int page,
            @RequestParam(defaultValue = "10", required = false) int pageSize,
        ...
        try {
            ...
            if (dateValidator.isValid(startDate) && dateValidator.isValid(endDate)) {
                ...
                Page<AnnouncementDto> announcementWithPage = announcementService.findAnnouncementByCompanyIdForAdmin(companyId,position,date1,date2,pageable);
                return listAnnouncement(announcementWithPage,response);
            } else {
    ...
    //TODO: Select announcement by id for Admin=========================================================
    @GetMapping("/announcements/admin/{id}")
    public ResponseEntity<BaseApiResponse<AnnouncementResponse>> findAnnouncementByIdForAdmin(@PathVariable int id) {

        BaseApiResponse<AnnouncementResponse> baseApiResponse = new BaseApiResponse<>();

        AnnouncementDto announcementDto = announcementService.findAnnouncementByIdForAdmin(id);
    ...
    public ResponseEntity<BaseApiResponse<AnnouncementResponse>> findAnnouncementByIdNotBanned(@PathVariable int id) {
        ...
        try {
            ...
            if (dateValidator.isValid(announcementRequest.getClosedDate()) && dateValidator.isValid(announcementRequest.getPublishedDate())) {
                ...
                if(company!=null){
                    ...
                    baseApiResponse.setTime(new Timestamp(System.currentTimeMillis()));
                    baseApiResponse.setStatus(HttpStatus.BAD_REQUEST);
                    baseApiResponse.setMessage("Company not found in database!");
                    baseApiResponse.setData(null);
                    return ResponseEntity.ok(baseApiResponse);
                }
    ...
    @GetMapping("/announcements/company/active-announcement-by-date/{companyId}")
    public ResponseEntity<Object> selectActiveAnnouncementByCompanyStartAndEndDate(
            @PathVariable int companyId,
            @RequestParam(defaultValue = "0", required = false) int page,
            @RequestParam(defaultValue = "10", required = false) int pageSize,
            @RequestParam(defaultValue = "2000-01-01", required = false) String startDate,
        ...
        try {
            ...
            if (dateValidator.isValid(startDate) && dateValidator.isValid(endDate)) {
                ...
                Page<AnnouncementDto> announcementWithPage = announcementService.findActiveAnnouncementByCompanyIdStartDateAndEndDate(companyId,date1,date2,pageable);
               return listAnnouncement(announcementWithPage,response);
            } else {
                baseApiResponse.setMessage("Please input with date format (yyyy-MM-dd)");
                baseApiResponse.setData(null);
    ...
    @GetMapping("/announcements/company/closed-announcement-by-date/{companyId}")
    public ResponseEntity<Object> selectCloseAnnouncementByCompanyStartAndEndDate(
            @PathVariable int companyId,
            @RequestParam(defaultValue = "0", required = false) int page,
            @RequestParam(defaultValue = "10", required = false) int pageSize,
            @RequestParam(defaultValue = "2000-01-01", required = false) String startDate,
        ...
        try {
            ...
            if (dateValidator.isValid(startDate) && dateValidator.isValid(endDate)) {
                ...
                Page<AnnouncementDto> announcementWithPage = announcementService.findCloseAnnouncementByCompanyIdStartDateAndEndDate(companyId,date1,date2,pageable);
                return listAnnouncement(announcementWithPage,response);
            } else {
                baseApiResponse.setMessage("Please input with date format (yyyy-MM-dd)");
                baseApiResponse.setData(null);
    ...
    @GetMapping("/announcements/company/draft-by-date/{companyId}")
    public ResponseEntity<Object> selectSaveDraftAnnouncementByCompanyStartAndEndDate(
            @PathVariable int companyId,
            @RequestParam(defaultValue = "0", required = false) int page,
            @RequestParam(defaultValue = "10", required = false) int pageSize,
            @RequestParam(defaultValue = "2000-01-01", required = false) String startDate,
        ...
        try {
            ...
            if (dateValidator.isValid(startDate) && dateValidator.isValid(endDate)) {
                ...
                Page<AnnouncementDto> announcementWithPage = announcementService.findSaveAnnouncementByCompanyIdStartDateAndEndDate(companyId,date1,date2,pageable);
                return listAnnouncement(announcementWithPage,response);
            } else {
                baseApiResponse.setMessage("Please input with date format (yyyy-MM-dd)");
                baseApiResponse.setData(null);
    ...
    @GetMapping("announcements/company/count-active/{companyId}")
    public ResponseEntity<BaseApiResponse<Map<String, Integer>>> countActiveAnnouncement(
            @PathVariable int companyId
    ) {
    ...
    @GetMapping("announcements/company/count-close/{companyId}")
    public ResponseEntity<BaseApiResponse<Map<String, Integer>>> countCloseAnnouncement(
            @PathVariable int companyId
    ) {
        Map<String, Integer> map = new HashMap<>();
        BaseApiResponse<Map<String, Integer>> baseApiResponse = new BaseApiResponse();
        int count = announcementService.countCloseAnnouncement(companyId, util.getCurrentDate());

        map.put("count", count);
        baseApiResponse.setData(map);
        baseApiResponse.setMessage("count close announcement successfully!");
        baseApiResponse.setStatus(HttpStatus.FOUND);
        baseApiResponse.setTime(new Timestamp(System.currentTimeMillis()));
        return ResponseEntity.ok(baseApiResponse);
    }
