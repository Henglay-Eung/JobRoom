...
public class ScheduleController {
    ...
    public ResponseEntity<BaseApiResponse<List<SchedulePostResponse>>> insertSchedule(@Valid @RequestBody SchedulePostRequest schedulePostRequest
                                ,String email,String password,String companyName) throws MessagingException {
        ...
        for (int i=0;i<schedulePostRequest.getCandidateIdRequest().length;i++) {
            ...
            scheduleDto.setMeetingTime(schedulePostRequest.getMeetingTime());
            scheduleDto.setEmailContent(schedulePostRequest.getEmailContent());
            scheduleResponse=modelMapper.map(scheduleService.insert(scheduleDto),SchedulePostResponse.class);
            scheduleService.notifyCandidate(candidateEmails.get(i),email,password,schedulePostRequest.getEmailContent(),companyName,schedulePostRequest.getPosition(),schedulePostRequest.getMeetingDate());
    ...
    @GetMapping("/schedules/{hrId}")
    public ResponseEntity<Object> findAllByHr(
            @PathVariable(name = "hrId") int id,
    ...
    public ResponseEntity<Object> updateSchedule(@PathVariable int id,@RequestBody SchedulePostRequest schedulePostRequest
        ...
        for (int i=0;i<schedulePostRequest.getCandidateIdRequest().length;i++) {
            ...
            scheduleDto.setMeetingTime(schedulePostRequest.getMeetingTime());
            scheduleDto.setEmailContent(schedulePostRequest.getEmailContent());
            scheduleResponse=modelMapper.map(scheduleService.update(id,scheduleDto),SchedulePostResponse.class);
            scheduleService.notifyCandidate(candidateEmails.get(0),email,password,schedulePostRequest.getEmailContent(),companyName,schedulePostRequest.getPosition(),schedulePostRequest.getMeetingDate());
    ...
    @GetMapping("/schedules/count/{hrId}")
    public ResponseEntity<BaseApiResponse<Map<String, Integer>>> count(
            @PathVariable int hrId
    ){
        Map<String,Integer> map=new HashMap<>();
        BaseApiResponse<Map<String,Integer>> baseApiResponse=new BaseApiResponse();
        int count =scheduleService.count(hrId);

        map.put("count",count);
        baseApiResponse.setData(map);
        baseApiResponse.setMessage("count schedule successfully!");
        baseApiResponse.setStatus(HttpStatus.FOUND);
        baseApiResponse.setTime(new Timestamp(System.currentTimeMillis()));
        return ResponseEntity.ok(baseApiResponse);
    }
    //TODO Count Schedule By HR
    @GetMapping("/schedules/by/{id}")
    public ResponseEntity<Object> findById(@PathVariable int id){

        ScheduleCandidate scheduleCandidate1= scheduleService.selectCandidateById(66);
        System.out.println(scheduleCandidate1);

        BaseApiResponse<ScheduleDto> baseApiResponse=new BaseApiResponse();
        ScheduleDto schedule =scheduleService.selectByIdAndStatus(id);
        if(schedule!=null){
            baseApiResponse.setData(schedule);
            baseApiResponse.setMessage(messageProperties.selectedOne("Schedule"));
            baseApiResponse.setStatus(HttpStatus.FOUND);
            baseApiResponse.setTime(new Timestamp(System.currentTimeMillis()));
            return ResponseEntity.ok(baseApiResponse);
        }else{
            baseApiResponse.setData(null);
            baseApiResponse.setMessage(messageProperties.hasNoRecord("Schedule"));
            baseApiResponse.setStatus(HttpStatus.NO_CONTENT);
            baseApiResponse.setTime(new Timestamp(System.currentTimeMillis()));
            return ResponseEntity.ok(baseApiResponse);
        }
